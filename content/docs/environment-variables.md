...
| variable | meaning |
| ----- | ----- |
| `INCLUDE_DRAFTS` | When this is set to `true`, draft posts will be included in the build. This overrides the normal behaviour, which is to include drafts when [ELEVENTY_RUN_MODE][] is set to `watch` or `serve` and to exclude them when ELEVENTY_RUN_MODE is set to `build`. Read more [about drafts here][drafts]. |
