...
---
When you add **Podcaster** to your Eleventy configuration file, you can pass in a number of options to modify its behaviour.
...
| `feedScript` | undefined | If you supply the absolute path or URL of a JavaScript file here, **Podcaster** will include a `<script>` tag in the feed, which you can use to style the feed when it is viewed in the browser. (New in v2.1) |
| `feedStylesheet` | undefined | If you supply the absolute path or URL of an XSL file here, **Podcaster** will include an `<?xml-stylesheet?>` processing instruction in the feed, which you can use to style the feed when it is viewed in the browser. (New in v2.1) |
...
| `validation` | undefined | When this is set to `true`, **Podcaster** checks if any data is missing that is required for the podcast feed. If it finds that data is missing it will stop the build in `build` mode and will log a warning in `serve` or `watch` modes. |
